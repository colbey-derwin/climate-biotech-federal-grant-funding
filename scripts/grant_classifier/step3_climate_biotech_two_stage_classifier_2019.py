...
The script starts toggled to TEST MODE to run the full year change to Test Mode = False 
...
# Adjust .parent levels based on where script is actually located
SCRIPT_DIR = Path(__file__).resolve().parent
PROJECT_ROOT = SCRIPT_DIR.parent.parent  # If in scripts/grant_classifier/
...
# Input file (from climate_biotech_filter output)
INFILE = OUTPUT_DIR / "climate_biotech_filtered.csv"

# Stage 1 outputs
OUT_STAGE1_ALL = OUTPUT_DIR / "stage1_biotech_fit.csv"
OUT_STAGE1_EXCL = OUTPUT_DIR / "stage1_excluded.csv"
OUT_STAGE1_REVIEW = OUTPUT_DIR / "stage1_review.csv"

# Stage 2 outputs
OUT_STAGE2_CHAR = OUTPUT_DIR / "stage2_characterized.csv"
OUT_STAGE2_REVIEW = OUTPUT_DIR / "stage2_review.csv"

# Log
OUT_LOG = OUTPUT_DIR / "two_stage_classification_log.json"
...
- If developing organisms to produce polymers or materials → biopolymers_materials
...
- biopolymers_materials - Bioplastics, biodegradable polymers, bio-based materials, living materials. The grant produces the FINAL polymer/material, not just the chemical building block.
