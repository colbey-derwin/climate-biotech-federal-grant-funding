...
# -*- coding: utf-8 -*-
"""
generate_sankey_COUNT_VERSION.py

Count-based version of the Sankey diagram.
Shows % of grant COUNT instead of % of funding amount.

Usage:
  python generate_sankey_COUNT_VERSION.py
...
OUTFILE = SCRIPT_DIR / "climate_biotech_sankey_count.html"
# =============================================================================
...
# Build links - COUNT VERSION (count grants instead of sum funding)
...
for _, row in df.iterrows():
    ...
    if source not in left_nodes:
        continue
    
    # Check each characteristic and create flow if present
    # COUNT: Each grant adds 1 to the count (not funding amount)
...
# Calculate grant counts for each left-side category
left_node_totals = {}
for node_name in left_nodes:
    # COUNT: Count grants in this category
    count = len(df[df['grant_category'] == node_name])
    left_node_totals[node_name] = count
    print(f"  {node_name}: {count:,} grants")

# Verify flows
print("\n" + "="*80)
print("FLOW VERIFICATION (COUNT):")
print("="*80)
for source_name in left_nodes:
    ...
    print(f"\n{source_name}:")
    print(f"  Total grants: {source_total:,}")
    print(f"  Total flows: {total_outflow:,} ({pct:.1f}%)")
    
    for link in source_links:
        link_count = link['value']
        print(f"    → {link['target']}: {link_count:,} grants")
...
<head>
    <meta charset="utf-8">
    <title>Climate Biotech Grant Flow (Count)</title>
...
<body>
    <div id="chart">
        <div class="chart-title">Climate Biotech Federal Grant Flow by Count (2019-2025)</div>
        <div class="chart-subtitle">Shows distribution of grant count across funding types and characteristics</div>
        <svg id="sankey"></svg>
    </div>
    <div id="tooltip"></div>
    ...
    <script>
        ...
        svg.append("g")
            ...
            .on("mouseover", function(event, d) {
                ...
                tooltip.style("opacity", 1)
                    .html(`${d.source.name} → ${d.target.name}<br>${formattedCount} grants (${pct}% of ${d.source.name})`)
       ...
       // Draw nodes
        ...
        node.append("rect")
            ...
            .attr("fill", d => d.color)
            .on("mouseover", function(event, d) {
                // Show tooltip with total count for this node
                if (d.actualTotal !== undefined) {
                    const formattedCount = d.actualTotal.toLocaleString();
                    
                    tooltip.style("opacity", 1)
                        .html(`${d.name}<br>${formattedCount} total grants`)
                        .style("left", (event.pageX + 10) + "px")
                        .style("top", (event.pageY - 20) + "px");
                }
            })
...
print("\n" + "="*80)
print("COUNT-BASED SANKEY GENERATED")
print("="*80)
print("\nChanges from funding version:")
print("  ✓ Shows grant COUNT instead of funding amount")
print("  ✓ Percentages reflect % of grant count")
print("  ✓ Tooltips show 'N grants' instead of '$N'")
print("  ✓ Title indicates 'by Count'")
print()
