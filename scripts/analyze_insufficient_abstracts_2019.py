...
# -*- coding: utf-8 -*-
"""
Analyze grants with INSUFFICIENT ABSTRACTS (<90 characters) by funding agency.
...
PROJECT_ROOT = SCRIPT_DIR.parent  # Assuming script is in scripts/

# Input/Output directories
OUTPUT_DIR = PROJECT_ROOT / "scripts" / "grant_classifier" / "output"

KEPT_FILE = OUTPUT_DIR / "climate_biotech_filtered.csv"
INSUFFICIENT_FILE = OUTPUT_DIR / "climate_biotech_insufficient_abstract.csv"

print("="*70)
print("INSUFFICIENT ABSTRACT ANALYSIS BY AGENCY")
print("Grants that passed climate × bio filter but have <90 char abstracts")
print("="*70)
...
print(f"\nKept (sufficient abstracts ≥90 chars):")
...
print(f"\nRemoved (insufficient abstracts <90 chars):")
...
print("-"*95)
for agency, row in comparison.iterrows():
...
try:
    ...
    kept = pd.DataFrame()
    have_kept_data = False
    print("\n⚠️  Could not load kept file for department totals")
...
for agency_pattern in ['DOE', 'DEPARTMENT OF ENERGY', 'DOD', 'DEPARTMENT OF DEFENSE']:
    ...
    if len(agency_insuf) > 0:
        ...
        for idx, row in agency_insuf.head(5).iterrows():
            ...
            print(f"    {row['award_id']}: {row['title'][:60]}...")
